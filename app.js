...
new Promise(async (resolve, reject) => {
    try {
      ...
      const nbrFlastText = await page.evaluate(() => document.querySelector('span.bold:nth-child(1)').innerText.split(' '));
          ...
          try {
            ...
            items.forEach((item) => {
              results.push({
                ...
                price: item.querySelector('ul.list-unstyled > li:nth-child(4) > span:nth-child(1)').innerText,
                link: item.querySelector('.list-item-link').getAttribute('href'),
                address: item.querySelector('.address').innerText,
                availability: item.querySelector('span:nth-child(2) > ul:nth-child(1) > li:nth-child(1)').innerText
              });
            });
            return results;
          }
