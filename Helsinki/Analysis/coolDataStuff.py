...
m = folium.Map([60.192059,24.945831], zoom_start=12, tiles='Stamen Watercolor')
...
for i,row in df.iterrows():
    ...
    house_string = '{:.0f} - {:.0f}m2\n {}'.format(row.price, row['size'], row.address)
    marker = folium.CircleMarker(location=[row.latitude,row.longitude], radius=(row.nbr_rooms*5), color=html, fill=html, fill_opacity='0.3', popup=house_string)
